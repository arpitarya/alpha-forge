"""Phase 4.2 — LightGBM Classifier for Stock Screener.

Primary ML model. Trains a LightGBM classifier to predict which stocks will
return >5% in the next 5 trading days.

Key design decisions:
- Uses walk-forward CV (never random split for time-series)
- Handles class imbalance via scale_pos_weight
- Handles NaN features natively (LightGBM supports missing values)
- Saves trained model + metadata to models/saved/
...
def compute_scale_pos_weight(y: np.ndarray) -> float:
    ...
    ratio = n_negative / n_positive — tells LightGBM to weight
    positive examples more heavily.
    """
    n_pos = (y == 1).sum()
    n_neg = (y == 0).sum()
    if n_pos == 0:
...
def create_lightgbm_model(params: dict | None = None, scale_pos_weight: float | None = None):
    """Create a LightGBM classifier instance.

    Args:
        params: Custom hyperparameters (merged with defaults).
        scale_pos_weight: Class imbalance weight. Auto-computed if None.
...
def train_lightgbm(
    ...
    """Train LightGBM with walk-forward CV and optionally save final model.
    ...
    final_model = create_lightgbm_model(params, scale_pos_weight=scale_pw)
...
def save_model(result: dict) -> Path:
    """Save trained model + metadata to models/saved/.

    Saves:
    - lightgbm_model.txt (LightGBM native format)
    - lightgbm_meta.json (metadata, metrics, feature list)
    - lightgbm_importance.csv (feature importance)
    """
    import lightgbm as lgb

    save_dir = MODELS_DIR / "lightgbm"
    ...
    # Save metadata (metrics, params, feature list)
    meta = {
        "model_type": "lightgbm",
...
def load_model(model_dir: Path | None = None):
    """Load a saved LightGBM model + metadata.
...
if __name__ == "__main__":
    ...
    # Get features (drop all-NaN columns)
    ...
    print(f"\nLightGBM Walk-Forward CV Results ({agg['n_folds']} folds, {agg['total_test_rows']:,} test rows)")
    print("=" * 70)
    ...
    for _, row in top.iterrows():
        print(f"  {row['feature']:<30s} {row['importance_gain']:.0f}")
