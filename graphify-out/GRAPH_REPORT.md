...
## Corpus Check
- 163 files · ~377,229 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 1167 nodes · 1989 edges · 39 communities detected
- Extraction: 62% EXTRACTED · 38% INFERRED · 0% AMBIGUOUS · INFERRED: 764 edges (avg confidence: 0.59)
...
Cohesion: 0.04
Nodes (118): BaseLLMProvider, Abstract base class for LLM providers — all providers implement this., Interface every LLM provider adapter must implement., Return the provider's default model., Shared OpenAI-compatible completion call used by all providers., BaseLLMProvider, BenchmarkReport, BenchmarkResult (+110 more)

### Community 1 - "Community 1"
Cohesion: 0.04
Nodes (74): initial schema with pgvector memory  Revision ID: 640eee61bc50 Revises:  Create, upgrade(), ai_chat(), ai_screener(), AnalysisRequest, AnalysisResponse, analyze_stock(), ChatMessage (+66 more)
...
Cohesion: 0.05
Nodes (37): BrokerName, Enum, createLogger(), get_logger(), getLogger(), Centralized logging configuration for AlphaForge Python services.  Usage::, Return a child logger under the given *namespace*.      Example::          logge, Configure and return the application root logger.      Resolution order for ever (+29 more)
...
Cohesion: 0.08
Nodes (12): ABC, BaseBroker, BaseBroker, BrokerOrder, BrokerPosition, Abstract broker interface — all broker integrations implement this., Interface every broker adapter must implement., Zerodha Kite Connect broker adapter. (+4 more)
...
Cohesion: 0.1
Nodes (20): Base, Base, Async database engine and session factory., Alembic env.py — async migration runner., Holding, open_positions(), Order, order_history() (+12 more)
...
- **Thin community `Community 32`** (6 nodes): `security.py`, `create_access_token()`, `decode_access_token()`, `hash_password()`, `Security utilities — password hashing, JWT tokens.`, `verify_password()`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 33`** (5 nodes): `ThemeProvider.tsx`, `readPersisted()`, `ThemeProvider()`, `useTheme()`, `writePersisted()`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
...
- **Thin community `Community 58`** (2 nodes): `Solar Terminal Design Tokens`, `@alphaforge/solar-orb-ui README`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 88`** (1 nodes): `Dark Charcoal Typography`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 89`** (1 nodes): `Two-Tier Stacked Wordmark`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.

## Suggested Questions
_Questions this graph is uniquely positioned to answer:_

- **Why does `evaluate_walk_forward()` connect `Community 2` to `Community 7`?**
  _High betweenness centrality (0.129) - this node is a cross-community bridge._
- **Why does `LLMProvider` connect `Community 0` to `Community 12`, `Community 7`?**
